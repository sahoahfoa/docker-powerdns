...
RUN set -ex; \
    apk --update add libpq sqlite-libs libstdc++ libgcc mariadb-client mariadb-client-libs lua5.3 libcurl; \
    apk add --virtual build-deps \
      g++ make mariadb-dev postgresql-dev sqlite-dev curl boost-dev lua5.3-dev curl-dev; \
    curl -sSL https://downloads.powerdns.com/releases/pdns-$POWERDNS_VERSION.tar.bz2 | tar xj -C /tmp; \
    cd /tmp/pdns-$POWERDNS_VERSION; \
    ./configure --prefix="" --exec-prefix=/usr --sysconfdir=/etc/pdns \
      --with-modules="bind gmysql gpgsql gsqlite3" --with-lua; \
